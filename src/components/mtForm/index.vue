<template>
  ...
  >
    ...
    >
        ...
        <span
          :style="
            fieldFrom[item.field]
              ? ''
              : '    font-size: 13px;    color: rgb(204, 207, 214);'
          "
        >{{ fieldFrom[item.field] || '请选择' + item.name }}</span
        >
      </div>
      <div
      ...
      >
        <span
          :style="fieldFrom[item.field]? '': '    font-size: 13px;    color: rgb(204, 207, 214);'"
        >{{ fieldFrom[item.field] || '请选择' + item.name }}</span
        >
      </div>
      <el-select
      ...
      />
      <el-input
        v-if="item.formType == 'text' && !item.width1"
        v-model="fieldFrom[item.field]"
        :disabled="item.disabled"
        :maxlength="item.maxLength || 100"
        :placeholder="item.placeholder"
      ...
      />
      <el-input
        v-if="item.formType == 'number' && !item.width1"
        v-model="fieldFrom[item.field]"
        :disabled="item.disabled"
        :maxlength="item.maxLength || 100"
        :placeholder="item.placeholder"
      ...
      <div
        v-else-if="item.formType == 'seleteAll'"
        class="input-width"
        style="    display: flex;"
      >
        <el-select
      ...
      <el-date-picker
        ...
        :picker-options="{
          disabledDate(time) {
            return time.getTime() < new Date(fieldFrom['startTime']).getTime()
          }
        }"
        class="input-width"
        @click.native.stop="() => {}"
      ...
      >
        <el-radio
          v-for="(ite, ind) in item.setting"
          :label="ite[item.optionV]"
          :key="ind"
        >{{ ite[item.optionL] }}</el-radio
        >
      </el-radio-group>
      <el-input
  ...
  methods: {
    ...
    typevalu(row) {
      ...
      if (
        ...
        this.urlname == 'orderOutDutyUserName' ||
        this.urlname == 'orderInDutyUserName' ||
        this.urlname == 'goodsCategoryDutyUserName'
      ) {
        this.fieldFrom[this.urlname] = row.name
        this.commonChange(row, this.urlname)
      } else if (this.urlname == 'companyName') {
        this.fieldFrom['companyName'] = row.name
        this.commonChange(row, 'companyName')
      } else {
    ...
    opende(row, idnex, name) {
      if (
        name == 'dutyUserName' ||
        name == 'orderOutDutyUserName' ||
        name == 'orderInDutyUserName' ||
        name == 'goodsCategoryDutyUserName'
      ) {
        this.p = '请输入姓名'
        this.urlname = name
        this.row = row
    ...
    opende2(row, idnex, name) {
      debugger
      if (this.fieldFrom.parentName) {
        this.pointobj = { parentId: this.fieldFrom.parentId, parentName: this.fieldFrom.parentName }
      } else {
        this.pointobj = {}
      }
